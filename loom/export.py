...
def trace_to_html(data: dict, path: str = "session.loom.json") -> str:
    ...
    Studio is now the interactive debugger (``loom debug``) delivered as a static
    file, so there is one viewer to maintain. ``path`` is accepted for backwards
    compatibility but no longer affects the output -- the page carries the run
    inline. The reusable analyzer panels in this module (``_impact_map``,
    ``_data_flow``, ...) remain for the incident/autopsy bundles."""
